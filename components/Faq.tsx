...
const faqData = [
  {
    question: "What is VisioAI?",
    pertanyaan: "Apa itu VisioAI?",
    answer: "VisioAI is a powerful visual content generator that uses Google's Gemini AI. It's designed for marketers, e-commerce owners, and creators to generate high-quality lookbooks and product B-rolls in seconds, helping to elevate affiliate and marketing campaigns.",
    jawaban: "VisioAI adalah generator konten visual canggih yang menggunakan AI Gemini dari Google. Ini dirancang untuk para pemasar, pemilik e-commerce, dan kreator untuk menghasilkan lookbook dan B-roll produk berkualitas tinggi dalam hitungan detik, membantu meningkatkan kampanye afiliasi dan pemasaran."
  },
  {
    question: "Do I need to install anything?",
    pertanyaan: "Apakah saya perlu menginstal sesuatu?",
    answer: "No, VisioAI runs directly in your browser within the Google AI Studio environment. There's nothing to download or install.",
    jawaban: "Tidak, VisioAI berjalan langsung di browser Anda dalam lingkungan Google AI Studio. Tidak ada yang perlu diunduh atau diinstal."
  },
  {
  ...
  {
    question: "Is VisioAI free to use?",
    pertanyaan: "Apakah VisioAI gratis untuk digunakan?",
...
const FaqItem: React.FC<{ item: typeof faqData[0], isOpen: boolean, onClick: () => void }> = ({ item, isOpen, onClick }) => {
  return (
    <div className="border-b-2 border-[#FDF6F0]/20 last:border-b-0">
      <button
        onClick={onClick}
        className="w-full flex justify-between items-center text-left py-5 px-6 text-lg font-semibold text-[#FDF6F0] focus:outline-none"
        aria-expanded={isOpen}
      >
        <span className="flex flex-col">
          <span>{item.pertanyaan}</span>
          <em className="text-base font-medium text-[#FDF6F0]/70 -mt-1">{item.question}</em>
        </span>
        <svg
      ...
      >
        <div className="overflow-hidden">
          <div className="px-6 pb-5 text-[#FDF6F0]/90 space-y-2">
            <p>{item.jawaban}</p>
            <p className="italic text-sm text-[#FDF6F0]/60">{item.answer}</p>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <section className="mt-16 md:mt-24">
      <div className="w-full">
        ...
        <div className="bg-[#6D597A] rounded-xl border-2 border-[#6D597A] shadow-[8px_8px_0px_#B56576]">
