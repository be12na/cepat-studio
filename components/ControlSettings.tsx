      ...
      <div>
        ...
        <div className="relative">
          <select
            ...
            className="w-full bg-white border-2 border-[#6D597A] text-[#3A3A3A] rounded-lg p-2.5 focus:ring-2 focus:ring-[#E56B6F] focus:border-[#E56B6F] transition appearance-none pr-8"
          >
            {themeOptions.map((option) => (
              <option key={option} value={option}>{option}</option>
            ))}
          </select>
           <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3 text-[#6D597A]">
