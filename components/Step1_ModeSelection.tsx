...
const ModeOption: React.FC<ModeOptionProps> = ({ title, description, onClick }) => (
  <button
    onClick={onClick}
    className="relative text-left w-full p-6 bg-white/50 rounded-xl border-2 border-[#6D597A] space-y-2 shadow-[8px_8px_0px_#B56576] transition-all duration-200 ease-in-out hover:shadow-[4px_4px_0px_#B56576] hover:translate-x-1 hover:translate-y-1 focus:outline-none focus:ring-4 focus:ring-[#E56B6F]/50"
  >
    <h3 className="text-xl font-bold text-[#6D597A]">{title}</h3>
...
export const Step1_ModeSelection: React.FC<Step1_ModeSelectionProps> = ({ onModeSelect }) => {
  return (
    <div className="w-full bg-white/50 border-2 border-[#6D597A] rounded-xl shadow-[8px_8px_0px_#6D597A] p-6 md:p-8">
