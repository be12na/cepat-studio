...
export const ImageUploader: React.FC<ImageUploaderProps> = ({ id, title, description, onImageUpload, onImageRemove, previewUrl }) => {
  ...
  const uploaderClasses = `relative flex flex-col items-center justify-center w-full aspect-square border-2 border-dashed rounded-lg cursor-pointer transition-colors duration-300 bg-[#FDF6F0] ${isDragging ? 'border-[#E56B6F] bg-[#FDF6F0]' : 'border-[#6D597A]/50 hover:border-[#6D597A]'}`;
  ...
  return (
    <div className="rounded-lg border-2 border-[#6D597A] overflow-hidden flex flex-col">
      <div className="bg-[#6D597A] text-[#FDF6F0] p-4">
        <h3 className="text-lg font-semibold">{title}</h3>
        <p className="text-sm text-[#FDF6F0]/80">{description}</p>
      </div>
      <div className="p-4">
        <label
          htmlFor={id}
          ...
          {previewUrl ? (
            <>
              ...
              <button
                ...
                className="absolute top-2 right-2 bg-[#6D597A]/80 hover:bg-[#6D597A] text-[#FDF6F0] rounded-full p-1.5 transition-colors"
